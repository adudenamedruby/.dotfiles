...
:END:
Make elisp in this file behave like we expect these days. Everyone has this set, but no one explains why. In non-elisp speak, it adds proper scoping and “closure” behaviour to variables. This [[https://www.emacswiki.org/emacs/DynamicBindingVsLexicalBinding][Emacswiki article]] explains it well.

Furthermore, to comply with Emacs conventions for libraries, the tangled ~init.el~ must have[[#h:0EDF1757-169B-4750-8262-415B599A4B53][the following header]].
...
:END:
Since I will be letting ~evil~ use ~C-u~, I should rebind ~universal-argument to something else. It will be ~C-M-u~
#+begin_src emacs-lisp
(use-package emacs
  :init
